...
function FadeIn({
  ...
  return (
    <motion.div ref={ref} initial={initial} animate={inView ? { opacity: 1, y: 0, x: 0 } : initial} transition={{ duration: 0.7, delay, ease: [0.25, 0.46, 0.45, 0.94] }} className={className}>
      {children}
    </motion.div>
  );
}
...
export default function IndustryPage() {
  ...
  const currentIdx = industries.findIndex((i) => i.slug === slug);
  const prev = currentIdx > 0 ? industries[currentIdx - 1] : null;
    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="relative mx-auto w-full max-w-6xl">
          ...
          <FadeIn delay={0.1}>
            <div className="mt-6 flex items-start gap-3 md:gap-5 md:items-center">
              <div className="flex h-12 w-12 md:h-16 md:w-16 flex-shrink-0 items-center justify-center rounded-2xl border border-white/[0.08]" style={{ backgroundColor: `${industry.color}15` }}>
                <Icon className="h-6 w-6 md:h-8 md:w-8" style={{ color: industry.color }} />
              </div>
              <h1 className="text-2xl sm:text-4xl font-bold tracking-tight md:text-6xl break-words">{industry.title}</h1>
            </div>
          </FadeIn>
          <FadeIn delay={0.2}><p className="mt-4 max-w-2xl text-base sm:text-xl font-medium md:text-2xl" style={{ color: industry.color }}>{industry.headline}</p></FadeIn>
          <FadeIn delay={0.3}><p className="mt-6 max-w-3xl text-base leading-relaxed text-neutral-400 md:text-lg">{industry.description}</p></FadeIn>
          <FadeIn delay={0.4}>
      ...
      <section className="relative px-4 pb-16 md:px-6 md:pb-24">
        <div className="mx-auto w-full max-w-6xl">
          <FadeIn><p className="text-xs font-medium uppercase tracking-[0.4em]" style={{ color: `${industry.color}cc` }}>Challenges</p></FadeIn>
          <FadeIn delay={0.1}><h2 className="mt-4 text-3xl font-semibold md:text-4xl">Industry challenges we solve</h2></FadeIn>
              ...
              <FadeIn key={c.title} delay={0.1 + i * 0.08}>
                <div className="group relative h-full rounded-2xl border border-white/[0.06] bg-white/[0.02] p-7 transition-all duration-300 hover:border-white/[0.12] hover:bg-white/[0.04]">
                  <div className="flex items-start gap-4">
                    <div className="mt-0.5 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: `${industry.color}18` }}>
                      <CheckCircle2 className="h-4 w-4" style={{ color: industry.color }} />
                    </div>
                    <div>
      ...
      <section className="relative px-4 py-16 md:px-6 md:py-24">
        <div className="mx-auto w-full max-w-6xl">
          <FadeIn><p className="text-xs font-medium uppercase tracking-[0.4em]" style={{ color: `${industry.color}cc` }}>Capabilities</p></FadeIn>
          <FadeIn delay={0.1}><h2 className="mt-4 text-3xl font-semibold md:text-4xl">What we deliver</h2></FadeIn>
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            {industry.capabilities.map((cap, i) => (
              <FadeIn key={cap} delay={0.1 + i * 0.06} direction="right">
                <div className="flex items-start gap-4 rounded-xl border border-white/[0.06] bg-white/[0.02] px-6 py-5">
                  <span className="mt-1 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold" style={{ backgroundColor: `${industry.color}18`, color: industry.color }}>
                    {i + 1}
                  </span>
                  <span className="text-sm leading-relaxed text-neutral-300 md:text-base">{cap}</span>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section className="relative px-4 py-16 md:px-6 md:py-24">
        <div className="mx-auto w-full max-w-6xl">
          <FadeIn><p className="text-xs font-medium uppercase tracking-[0.4em]" style={{ color: `${industry.color}cc` }}>Compliance</p></FadeIn>
          <FadeIn delay={0.1}><h2 className="mt-4 text-3xl font-semibold md:text-4xl">Frameworks we work with</h2></FadeIn>
          <div className="mt-10 flex flex-wrap gap-3">
            {industry.compliance.map((c, i) => (
              <FadeIn key={c} delay={0.05 + i * 0.04}>
                <span className="inline-flex items-center gap-2 rounded-full border px-5 py-2.5 text-sm font-medium transition-colors hover:bg-white/[0.06]" style={{ borderColor: `${industry.color}30`, color: industry.color }}>
        ...
        <div className="mx-auto w-full max-w-6xl">
          <FadeIn>
            <div className="relative overflow-hidden rounded-2xl md:rounded-3xl border border-white/[0.08] bg-gradient-to-br from-white/[0.04] to-transparent p-6 sm:p-10 md:p-16">
              <div className="pointer-events-none absolute -right-32 -top-32 h-64 w-64 rounded-full blur-[100px]" style={{ backgroundColor: `${industry.color}15` }} />
              <div className="relative z-10">
                <h2 className="max-w-2xl text-3xl font-semibold leading-tight md:text-4xl">
                  Building for <span className="bg-clip-text text-transparent" style={{ backgroundImage: `linear-gradient(to right, ${industry.color}, white)` }}>{industry.title.toLowerCase()}</span>?
                </h2>
